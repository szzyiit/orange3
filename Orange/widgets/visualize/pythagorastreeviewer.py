...
class PythagorasTreeViewer(QGraphicsWidget):
    ...
    def set_size_calc(self, weight_adjustment):
        ...
        self.weight_adjustment = weight_adjustment
        self.set_tree(
            self.tree_adapter, self.weight_adjustment, self._target_class_index
        )
...
class InteractiveSquareGraphicsItem(SquareGraphicsItem):
    ...
    def mouseDoubleClickEvent(self, event):
        ...
        p.set_tree(
            p.tree_adapter, p.weight_adjustment, self.tree_node.target_class_index
        )
...
class DiscreteTreeNode(TreeNode):
    ...
    def tooltip(self):
        ...
        return (
            "<p>"
            + text
            + "{}/{} 个样本 ({:2.3f}%)".format(int(samples), total, ratio * 100)
            + "<hr>"
            + (
