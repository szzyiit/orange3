...
class TestOWDataSampler(WidgetTest):
    ...
    def test_error_message(self):
        ...
        self.send_signal("数据(Data)", self.iris[:5])
        self.assertTrue(self.widget.Error.too_many_folds.is_shown())
        self.send_signal("数据(Data)", None)
        self.assertFalse(self.widget.Error.too_many_folds.is_shown())
        self.send_signal("数据(Data)", Table.from_domain(self.iris.domain))
    ...
    def test_stratified_on_unbalanced_data(self):
        ...
        self.send_signal("数据(Data)", unbalanced_data)
    ...
    def test_bootstrap(self):
        ...
        sample = self.get_output("数据样本(Data Sample)")
        in_sample = set(sample.ids)
        in_remaining = set(self.get_output("剩余数据(Remaining Data)").ids)
        ...
        for replicable in [True, False]:
            for stratified in [True, False]:
                for sampling_type in sampling_types:
                    ...
                    sample = self.get_output("数据样本(Data Sample)")
                    other = self.get_output("剩余数据(Remaining Data)")
    ...
    def test_bigger_size_with_replacement(self):
        ...
        self.send_signal("数据(Data)", self.iris[:2])
